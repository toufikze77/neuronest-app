-- NeuroNest: Complete Clean Setup (Handles Dependencies)
-- This script safely removes ALL existing tables and policies, then recreates everything

-- Drop all RLS policies first (dependencies)
DROP POLICY IF EXISTS "Admin full access to votes" ON public.votes;
DROP POLICY IF EXISTS "Users can manage their own votes" ON public.votes;
DROP POLICY IF EXISTS "Admin full access to comments" ON public.comments;
...
DROP POLICY IF EXISTS "Members can view comments on private community posts" ON public.comments;
...
DROP POLICY IF EXISTS "Anyone can view posts in public communities" ON public.posts;
DROP POLICY IF EXISTS "Admin full access to community members" ON public.community_members;
DROP POLICY IF EXISTS "Users can leave communities" ON public.community_members;
DROP POLICY IF EXISTS "Users can join communities" ON public.community_members;
DROP POLICY IF EXISTS "Members can view community membership" ON public.community_members;
DROP POLICY IF EXISTS "Admin full access to communities" ON public.communities;
DROP POLICY IF EXISTS "Owners can update their communities" ON public.communities;
DROP POLICY IF EXISTS "Authenticated users can create communities" ON public.communities;
DROP POLICY IF EXISTS "Members can view private communities they belong to" ON public.communities;
...
DROP POLICY IF EXISTS "Public can view verified active profiles" ON public.profiles;
...
DROP INDEX IF EXISTS votes_user_comment_idx;
DROP INDEX IF EXISTS votes_user_post_idx;
DROP INDEX IF EXISTS community_members_user_idx;
DROP INDEX IF EXISTS community_members_community_idx;
DROP INDEX IF EXISTS posts_created_idx;
DROP INDEX IF EXISTS posts_community_idx;
DROP INDEX IF EXISTS posts_author_idx;
DROP INDEX IF EXISTS communities_owner_idx;
DROP INDEX IF EXISTS communities_slug_idx;
DROP INDEX IF EXISTS profiles_active_verified_idx;
