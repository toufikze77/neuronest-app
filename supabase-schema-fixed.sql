-- NeuroNest Secure Database Schema for Multitenant SaaS
...
-- Community members table (must be created before RLS policies that reference it)
...
CREATE POLICY "Members can view private communities they belong to" ON public.communities
  FOR SELECT USING (
    is_private = true AND 
    EXISTS (
...
CREATE INDEX IF NOT EXISTS profiles_active_verified_idx ON public.profiles(is_active, email_verified);
...
CREATE INDEX IF NOT EXISTS votes_user_post_idx ON public.votes(user_id, post_id);
CREATE INDEX IF NOT EXISTS votes_user_comment_idx ON public.votes(user_id, comment_id);
